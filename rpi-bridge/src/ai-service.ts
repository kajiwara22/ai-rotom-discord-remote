 ...
 *
 * "low" でも複雑な対戦分析では思考が 5,000 トークンに達し、1 問あたり 2 分半
 * かかっていたため "none" にする。ツール選択の判断が落ちる懸念はあったが、
 * list_parties → load_party のような単純な判断ではモデル自身が思考なしで
 * 処理していることを実測で確認した。回答品質が落ちるようなら "low" に戻す。
