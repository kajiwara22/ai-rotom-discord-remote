...
export const TOOL_DEFINITIONS: ToolDefinition[] = [
  ...
  },
  {
    ...
    function: {
      name: "get_match",
      description:
        "1つの対戦の記録を取得する。相手の構築6体・相手の先発・自分のパーティ名（party）と6体構成（team）・自分の先発と選出・勝敗・動画の該当位置が分かる。選出・先発の体数は battleFormat に応じ、シングルは選出 3 体／先発 1 体、ダブルは選出 4 体／先発 2 体。技やダメージなどの行動ログは記録されていないため、選出や構築相性の振り返りに使うこと。持ち物・技などのパーティ詳細が必要なら、party を load_party に渡して保存済みパーティを取得する。",
      parameters: {
        type: "object",
        properties: {
          matchId: {
...
- ツールが返す「もしかして: ○○」は名前が似ているだけの候補であり、ユーザーが意図したポケモンとは限りません。**勝手に別のポケモンとして話を進めないでください。** 「〇〇のことですか？」と確認してから進めること。
- パーティを扱うツール（analyze_selection、analyze_party_weakness など）は、メンバーに 1 体でも未収録が含まれるとパーティ全体がエラーになります。その場合はどのポケモンが未収録なのかを個別に調べて特定し、ユーザーに伝えてください。
...
## 応答スタイル
- 簡潔に回答する
- Markdown の太字、コードブロック、テーブル、リストを適宜使用する
- 日本語で回答する`;
}
