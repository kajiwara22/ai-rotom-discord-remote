...
AI に「このパーティを保存して」と頼むと、ai-rotom 側の `~/.ai-rotom/parties.json` に保存されます（会話履歴とは別のファイルです）。

**保存したパーティは利用者ごとに分かれています。** 兄弟が同じ「じぶんのパーティ」という名前で保存しても、お互いを上書きしません。
...
これはファイル内部でパーティ名に接頭辞を付けることで実現しています（→ [ADR-0009](docs/adr/ADR-0009.md)）。画面や AI の回答には接頭辞のない元の名前だけが出ます。

> `~/.ai-rotom/parties.json` を直接開くと `user:xxx/パーティ名` の形で並んでいますが、これは内部表現です。手で編集する場合はこの形式を保ってください。
